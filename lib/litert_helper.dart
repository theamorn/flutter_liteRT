...
class LiteRTHelper {
  ...
  Future<void> loadModel({
    ...
    try {
      ...
      if (useGpu) {
        try {
          if (Platform.isAndroid) {
            ...
            debugPrint(
              'GPU delegate enabled (Android) - using OpenGL/OpenCL GPU acceleration',
            );
      ...
      debugPrint(
        'IsolateInterpreter created - inference will run in separate isolate',
      );
      ...
      debugPrint(
        'Output tensor - type: ${outputTensor.type}, classes: $numClasses',
      );
  ...
  Uint8List _preProcessImage(File imageFile) {
    ...
    final inputBytes = Uint8List(
      1 * inputImageSize * inputImageSize * inputChannels,
    );
    int pixelIndex = 0;
  ...
  List<Map<String, dynamic>> getTopPredictions(
    File imageFile, {
    int topN = 10,
  }) {
    ...
    final List<double> rawOutput =
        outputList.map((e) => (e as num).toDouble()).toList();
  ...
  Future<List<dynamic>> predictFromCameraImage(
    CameraImage image, {
    ...
    try {
      ...
      final preprocessResult = await _preprocessCameraImage(image, useIsolate: useIsolate);
      ...
      final inferenceTime =
          DateTime.now().difference(inferenceStart).inMilliseconds.toDouble();
  ...
  Future<Map<String, dynamic>> _preprocessCameraImage(
    CameraImage image, {
    bool useIsolate = true,
  }) async {
    ...
    final convertTime =
        DateTime.now().difference(convertStart).inMilliseconds.toDouble();
    ...
    if (useIsolate) {
      // Use compute to move heavy image processing to separate isolate (smoother but slower)
      inputBytes = await compute(
      ...
      // Direct resize in current isolate (faster but may cause UI jank)
      inputBytes = _resizeAndConvertImage(
        _ImageResizeParams(
          image: rgbImage,
          targetSize: inputImageSize,
          channels: inputChannels,
    ...
    final resizeTime =
        DateTime.now().difference(resizeStart).inMilliseconds.toDouble();
  ...
  img.Image _convertToImage(CameraImage cameraImage) {
    ...
    for (int h = 0; h < height; h++) {
      ...
      for (int w = 0; w < width; w++) {
        ...
        int g = (y - u * 0.34414 - v * 0.71414 + 135.45984).round().clamp(
          0,
          255,
        );
...
Uint8List _resizeAndConvertImage(_ImageResizeParams params) {
  ...
  final inputBytes = Uint8List(
    1 * params.targetSize * params.targetSize * params.channels,
  );
  int pixelIndex = 0;
